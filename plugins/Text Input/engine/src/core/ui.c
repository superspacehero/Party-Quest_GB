...
UBYTE ui_draw_text_buffer_char(void) BANKED
{
    ...
    if (ui_text_ptr == 0)
    {
        ...
        if ((text_options & TEXT_OPT_PRESERVE_POS) == 0)
        {
            ...
            if (vwf_direction == UI_PRINT_RIGHTTOLEFT)
                ui_dest_base += 17; // right_to_left initial pos correction
    ...
    while (TRUE)
    {
        ...
        default:
            ...
            if (vwf_current_offset)
                ui_set_tile(ui_dest_ptr, ui_current_tile, ui_current_tile_bank);
            ui_text_ptr++;
            return TRUE;
        }
        ui_text_ptr++;
...
void ui_update(void) NONBANKED
{
    ...
    {
        if (game_time & current_text_speed)
            return;
    }
    // render next char
    do
    ...
    // play sound
    if ((flag) && (text_sound_bank != SFX_STOP_BANK))
        music_play_sfx(text_sound_bank, text_sound_data, text_sound_mask, MUSIC_SFX_PRIORITY_NORMAL);
}
...
// UBYTE ui_input_text(keyboard_item_t *start_item, UBYTE bank, UBYTE options, UBYTE count, UBYTE start_index) BANKED
// {
//     keyboard_item_t current_key;
//     UBYTE current_index = ((options & MENU_SET_START) ? start_index : 1u), next_index = 0u;
//     // copy first menu item
//     MemcpyBanked(&current_key, start_item + (current_index - 1u), sizeof(menu_item_t), bank);
...
//     {
//         VBK_REG = 1;
//         set_win_tile_xy(current_key.X, current_key.Y, overlay_priority | (text_palette & 0x07u));
//         VBK_REG = 0;
//     }
// #endif
//     set_win_tile_xy(current_key.X, current_key.Y, ui_cursor_tile);
...
//         if (INPUT_UP_PRESSED)
//         {
//             next_index = current_key.iU;
//         }
//         else if (INPUT_DOWN_PRESSED)
//         {
//             next_index = current_key.iD;
//         }
//         else if (INPUT_LEFT_PRESSED)
//         {
//             next_index = current_key.iL;
//         }
//         else if (INPUT_RIGHT_PRESSED)
//         {
//             next_index = current_key.iR;
//         }
//         else if (INPUT_A_PRESSED)
//         {
//             return ((current_index == count) && (options & MENU_CANCEL_LAST)) ? 0u : current_index;
...
//         }
// #endif
//         set_win_tile_xy(current_key.X, current_key.Y, ui_bg_tile);
//         // read menu data
//         MemcpyBanked(&current_key, start_item + current_index - 1u, sizeof(menu_item_t), bank);
//         // put new cursor
// #ifdef CGB
